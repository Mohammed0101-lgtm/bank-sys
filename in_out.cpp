...
bool match(const std::string& name, const std::string& accountowner) {
    ...
    strippedAccountowner.erase(std::remove(strippedAccountowner.begin(), strippedAccountowner.end(), '\"'),
    ...
    for (const std::string& enteredToken : enteredTokens)
        if (std::find(accountownerTokens.begin(), accountownerTokens.end(), enteredToken) == accountownerTokens.end())
            return false;
