...
std::string bcryptHash(const std::string& password, const std::string& salt) {
    ...
    trimmedpassword.erase(std::remove_if(trimmedpassword.begin(), trimmedpassword.end(), ::isspace),
                          trimmedpassword.end());
    ...
    int result =
      PKCS5_PBKDF2_HMAC(password.c_str(), password.length(), reinterpret_cast<const unsigned char*>(salt.c_str()),
                        salt.length(), rounds, EVP_sha256(), hash.length(), reinterpret_cast<unsigned char*>(&hash[0]));
